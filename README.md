...
| 명령어/기능 | 설명 |
| --- | --- |
| `/지진알림` | 현재 채널에 일본 M5.9 이상 긴급지진속보 알림 설정/해제 |
| `/채널설정 기능:일본지진알림` | 지정한 채널에 일본 긴급지진속보 알림 설정/해제 |

`/지진알림 상태:true`로 실행한 현재 채널이나 `/채널설정 기능:일본지진알림 채널:#알림채널`로 지정한 채널에 알림을 보냅니다. 두 명령어는 같은 설정을 사용합니다. Wolfx의 JMA 긴급지진속보 WebSocket에 상시 연결하고 일본 기상청이 발표한 M5.9 이상 예보와 경보를 실시간으로 전달합니다. 진원지와 예상 지역의 일본어는 별도 키가 필요 없는 Google 번역으로 변환해 `일본어(한국어)` 형식으로 표시하며, 번역 요청이 실패하면 원문으로 알림을 계속 보냅니다. 추정 진원은 OpenStreetMap 지도 이미지와 크게 보기 링크로 표시합니다. 같은 지진의 후속보와 최종보는 기존 Discord 메시지와 지도 위치를 수정하며 취소보도 반영합니다. 훈련보와 2분 이상 지난 초기 스냅샷은 알리지 않습니다.

규모 M7.0 이상은 `@everyone`을 함께 전송합니다. 같은 지진에서는 한 번만 멘션하며, 후속보에서 처음 M7.0 이상으로 상향되면 멘션이 포함된 새 메시지로 교체합니다. 실제 알림을 보내려면 봇에 대상 채널의 `@everyone`, `@here`, 모든 역할 멘션 권한이 있어야 합니다.

Discord 임베드는 규모, 깊이, 최대 예상 진도와 발표 단계, 발표 시각, 발생 추정 시각을 각각 3열 행으로 표시합니다. 왼쪽 테두리는 규모 M3.0 이상 초록, M4.0 이상 노랑, M5.0 이상 주황, M6.0 이상 빨강이며 취소보와 규모 미상은 회색입니다.
