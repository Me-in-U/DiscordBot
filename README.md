...
`/지진알림 상태:true`로 실행한 현재 채널이나 `/채널설정 기능:일본지진알림 채널:#알림채널`로 지정한 채널에 알림을 보냅니다. 두 명령어는 같은 설정을 사용합니다. Wolfx의 JMA 긴급지진속보 WebSocket에 상시 연결하고 일본 기상청이 발표한 M4.0 이상 예보와 경보를 실시간으로 전달합니다. 같은 지진의 후속보와 최종보는 기존 Discord 메시지를 수정하며 취소보도 반영합니다. 훈련보와 2분 이상 지난 초기 스냅샷은 알리지 않습니다.
