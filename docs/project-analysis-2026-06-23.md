...
9. 현재 최신 검증은 `compileall` 통과, unittest 390개 통과다.
10. 이번 패치로 music favorite play action facade 분리가 구현되었고, 다음 후보는 music favorite manager selection action 분리다.
...
최신 검증 결과: `python -m compileall -q bot.py api cogs common func util test scripts` 통과, `python -m unittest discover -s test` 390개 통과.
...
이번 패치로 music favorite play action facade 분리가 완료되었다. 다음 작은 후보는 music favorite manager selection action 분리다.
...
| 작업트리 최신 검증 | `python -m compileall -q bot.py api cogs common func util test scripts` 통과, `python -m unittest discover -s test` -> 390개 통과 | 구현 진행 후 회귀 확인 |
...
| music View/Modal 분리 | `util/music_views.py` 405줄 추출, `cogs/music.py` 현재 1670줄, views 대상 테스트 3개 통과 | 검색 결과/즐겨찾기/control/helper View와 search/seek modal을 music Cog 본문에서 분리 |
...
| music favorite snapshot/payload/play helper 분리 | `util/music_favorites.py` 현재 304줄, `cogs/music.py` 현재 1670줄, favorite 대상 테스트 15개 통과 | 현재 재생 player를 즐겨찾기 snapshot으로 바꾸고, 검색 결과 entry와 현재 재생 snapshot을 저장 payload/action으로 정규화하며, 빈 슬롯/재생 URL action 판단을 music Cog 본문에서 분리 |
