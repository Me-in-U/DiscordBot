...
9. 현재 최신 검증은 `compileall` 통과, unittest 484개 통과다.
10. 이번 패치로 MapleStory parser helper가 `util/maplestory/` 카테고리 패키지로 이동되었고, MapleStory util 카테고리 정리가 이어졌다.
...
최신 검증 결과: `python -m compileall -q bot.py api cogs common func util test scripts` 통과, `python -m unittest discover -s test` 484개 통과.
...
이번 패치로 MapleStory parser helper의 `util/maplestory/` 패키지 이동이 완료되었다. root `util/music*.py` helper와 root `util/youtube*.py` helper는 더 이상 남지 않았고, MapleStory root util helper의 카테고리 이동이 계속 진행 중이다.
...
| 작업트리 최신 검증 | `python -m compileall -q bot.py api cogs common func util test scripts` 통과, `python -m unittest discover -s test` -> 484개 통과 | 구현 진행 후 회귀 확인 |
