...
최신 검증 결과: `python -m compileall -q bot.py api cogs common func util test scripts` 통과, `python -m unittest discover -s test` 540개 통과.
...
현재까지 guild channel settings helper의 `util/guild/` 패키지 이동과 Music/YouTube subscriptions/MapleStory/LoL scrim/DDAY/Channel settings/Party/Loop/Voice chat/Search/Questions/Explanation/Interpret/Translation/Summarize/Rank/Scheduler/Status API Cog package 이동이 완료되었다. root `util/music*.py`, root `util/youtube*.py`, root `util/maplestory*.py`, root loop helper, root celebration helper는 더 이상 남지 않았다.
...
- `cogs/`: Discord 명령과 백그라운드 태스크 구현
  ...
  - 패키지 Cog: `cogs/channel_settings/`, `cogs/dday/`, `cogs/explanation/`, `cogs/gambling/`, `cogs/interpret/`, `cogs/lol_scrim/`, `cogs/loop/`, `cogs/maplestory/`, `cogs/music/`, `cogs/party/`, `cogs/questions/`, `cogs/rank/`, `cogs/scheduler/`, `cogs/search/`, `cogs/status_api/`, `cogs/summarize/`, `cogs/translation/`, `cogs/voice_chat/`, `cogs/youtube_subscriptions/`
...
| 작업트리 최신 검증 | `python -m compileall -q bot.py api cogs common func util test scripts` 통과, `python -m unittest discover -s test` -> 540개 통과 | 구현 진행 후 회귀 확인 |
