...
9. 현재 최신 검증은 `compileall` 통과, unittest 554개 통과다.
10. 이번 패치로 `cogs/music/`, `cogs/youtube_subscriptions/`, `cogs/maplestory/`, `cogs/lol_scrim/`, `cogs/dday/`, `cogs/channel_settings/`, `cogs/party/`, `cogs/loop/`, `cogs/voice_chat/`, `cogs/search/`, `cogs/questions/`, `cogs/explanation/`, `cogs/interpret/`, `cogs/translation/`, `cogs/summarize/`, `cogs/rank/`, `cogs/scheduler/`, `cogs/status_api/`, `cogs/celebration_update/`, `cogs/exchange_rate/`, `cogs/foreign_reserves/`, `cogs/clean/`, `cogs/custom_help/`, `cogs/emoji_enlarge/`, `cogs/ping/` package cog 전환과 guild channel settings helper의 `util/guild/` 이동이 반영되었고, root util은 공통 infra 중심으로 축소되었다.
...
최신 검증 결과: `python -m compileall -q bot.py api cogs common func util test scripts` 통과, `python -m unittest discover -s test` 554개 통과.
...
| 완료 | WebSub helper 분리 | `util/youtube/websub.py`에 callback URL/payload helper 추가, `cogs/loop/__init__.py` WebSub 요청 조립 책임 축소 |
...
| 완료 | DDAY Cog package 이동 | `cogs/dday.py`를 `cogs/dday/__init__.py` package cog로 이동하고, `cogs.dday` 공개 import와 bot extension 이름은 유지하면서 물리 배치를 DDAY 카테고리 폴더로 통일 |
| 완료 | Channel settings Cog package 이동 | `cogs/channel_settings.py`를 `cogs/channel_settings/__init__.py` package cog로 이동하고, `cogs.channel_settings` 공개 import와 bot extension 이름은 유지하면서 물리 배치를 guild/channel settings 카테고리 폴더로 통일 |
| 완료 | Party Cog package 이동 | `cogs/party.py`를 `cogs/party/__init__.py` package cog로 이동하고, `cogs.party` 공개 import와 bot extension 이름은 유지하면서 물리 배치를 party 카테고리 폴더로 통일 |
...
| 완료 | Interpret Cog package 이동 | `cogs/interpret.py`를 `cogs/interpret/__init__.py` package cog로 이동하고, `cogs.interpret` 공개 import와 bot extension 이름은 유지하면서 물리 배치를 interpret 카테고리 폴더로 통일 |
| 완료 | Translation Cog package 이동 | `cogs/translation.py`를 `cogs/translation/__init__.py` package cog로 이동하고, `cogs.translation` 공개 import와 bot extension 이름은 유지하면서 물리 배치를 translation 카테고리 폴더로 통일 |
| 완료 | Summarize Cog package 이동 | `cogs/summarize.py`를 `cogs/summarize/__init__.py` package cog로 이동하고, `cogs.summarize` 공개 import와 bot extension 이름은 유지하면서 물리 배치를 summarize 카테고리 폴더로 통일 |
| 완료 | Rank Cog package 이동 | `cogs/rank.py`를 `cogs/rank/__init__.py` package cog로 이동하고, `cogs.rank` 공개 import와 bot extension 이름은 유지하면서 물리 배치를 rank 카테고리 폴더로 통일 |
| 완료 | Scheduler Cog package 이동 | `cogs/scheduler.py`를 `cogs/scheduler/__init__.py` package cog로 이동하고, `cogs.scheduler` 공개 import와 bot extension 이름은 유지하면서 물리 배치를 scheduler 카테고리 폴더로 통일 |
| 완료 | Status API Cog package 이동 | `cogs/status_api.py`를 `cogs/status_api/__init__.py` package cog로 이동하고, `cogs.status_api` 공개 import와 bot extension 이름은 유지하면서 물리 배치를 status API 카테고리 폴더로 통일 |
| 완료 | Celebration update Cog package 이동 | `cogs/celebration_update.py`를 `cogs/celebration_update/__init__.py` package cog로 이동하고, `cogs.celebration_update` 공개 import와 bot extension 이름은 유지하면서 물리 배치를 celebration update 카테고리 폴더로 통일 |
| 완료 | Exchange rate Cog package 이동 | `cogs/exchange_rate.py`를 `cogs/exchange_rate/__init__.py` package cog로 이동하고, `cogs.exchange_rate` 공개 import와 bot extension 이름은 유지하면서 물리 배치를 exchange rate 카테고리 폴더로 통일 |
| 완료 | Foreign reserves Cog package 이동 | `cogs/foreign_reserves.py`를 `cogs/foreign_reserves/__init__.py` package cog로 이동하고, `cogs.foreign_reserves` 공개 import와 bot extension 이름은 유지하면서 물리 배치를 foreign reserves 카테고리 폴더로 통일 |
| 완료 | Clean Cog package 이동 | `cogs/clean.py`를 `cogs/clean/__init__.py` package cog로 이동하고, `cogs.clean` 공개 import와 bot extension 이름은 유지하면서 물리 배치를 clean 카테고리 폴더로 통일 |
| 완료 | Custom help Cog package 이동 | `cogs/custom_help.py`를 `cogs/custom_help/__init__.py` package cog로 이동하고, `cogs.custom_help` 공개 import와 bot extension 이름은 유지하면서 물리 배치를 custom help 카테고리 폴더로 통일 |
| 완료 | Emoji enlarge Cog package 이동 | `cogs/emoji_enlarge.py`를 `cogs/emoji_enlarge/__init__.py` package cog로 이동하고, `cogs.emoji_enlarge` 공개 import와 bot extension 이름은 유지하면서 물리 배치를 emoji enlarge 카테고리 폴더로 통일 |
| 완료 | Ping Cog package 이동 | `cogs/ping.py`를 `cogs/ping/__init__.py` package cog로 이동하고, `cogs.ping` 공개 import와 bot extension 이름은 유지하면서 물리 배치를 ping 카테고리 폴더로 통일 |
...
| 완료 | Loop task lifecycle 1차 분리 | `util/loop/task_lifecycle.py`로 task start/cancel 정책 이동, `LoopTasks.cog_unload()`에서 실행 중인 loop cancel |
...
| 완료 | Loop Cog package 이동 | `cogs/loop.py`를 `cogs/loop/__init__.py` package cog로 이동하고, `cogs.loop` 공개 import와 bot extension 이름은 유지하면서 물리 배치를 loop 카테고리 폴더로 통일 |
...
| 완료 | YouTube video candidate runner 패키지 이동 | `util/youtube_video_candidate_runner.py`를 `util/youtube/video_candidate_runner.py`로 이동하고, `cogs.loop`/test import를 새 카테고리 패키지 경로로 통일했으며 root helper 제거를 경로 계약 테스트로 고정 |
...
| 완료 | YouTube notification sender 1차 분리 | `util/youtube/notification_sender.py`로 live/upload 알림 대상 채널 resolve와 메시지 전송 이동 |
| 완료 | YouTube notification sender helper 패키지 이동 | `util/youtube_notification_sender.py`를 `util/youtube/notification_sender.py`로 이동하고, `cogs.loop`/community notification/test import를 새 카테고리 패키지 경로로 통일했으며 root helper 제거를 경로 계약 테스트로 고정 |
...
| 완료 | Weekly 1557 report runner 1차 분리 | `util/loop/weekly_1557_reporter.py`로 주간 1557 리포트 생성/전송/초기화 orchestration 이동 |
| 완료 | Weekly 1557 report runner 패키지 이동 | `util/weekly_1557_reporter.py`를 `util/loop/weekly_1557_reporter.py`로 이동하고, `cogs.loop`/test import를 새 카테고리 패키지 경로로 통일했으며 root helper 제거를 경로 계약 테스트로 고정 |
| 완료 | Presence status helper 1차 분리 | `util/loop/presence_status.py`로 USER_MESSAGES 집계와 presence 문구 생성 이동 |
| 완료 | Presence status helper 패키지 이동 | `util/presence_status.py`를 `util/loop/presence_status.py`로 이동하고, `cogs.loop`/test import를 새 카테고리 패키지 경로로 통일했으며 root helper 제거를 경로 계약 테스트로 고정 |
| 완료 | MapleStory notice loop runner 1차 분리 | `util/maplestory/notice_loop_runner.py`로 공지 결과 집계/로그 orchestration 이동, 3분 polling loop는 `cogs/loop/__init__.py`에 유지 |
...
| 완료 | LoL scrim helper 패키지 이동 | `util/lol_scrim.py`를 `util/lol/scrim.py`로 이동하고, `cogs.lol_scrim`/test import를 새 카테고리 패키지 경로로 통일했으며 root helper 제거를 경로 계약 테스트로 고정 |
...
| 완료 | Guild channel settings helper 패키지 이동 | `util/channel_settings.py`를 `util/guild/channel_settings.py`로 이동하고, guild/channel 설정 호출부와 AGENTS 안내를 새 카테고리 패키지 경로로 통일했으며 root helper 제거를 경로 계약 테스트로 고정 |
| 완료 | Music Cog package 이동 | `cogs/music.py`를 `cogs/music/__init__.py` package cog로 이동하고, `cogs.music` 공개 import와 bot extension 이름은 유지하면서 물리 배치를 music 카테고리 폴더로 통일 |
...
| 완료 | music 상태성 `print`/logger 정리 | `cogs/music/__init__.py`의 direct `print()`를 제거하고 debug/status 출력이 `logger.debug/info`를 통과하도록 AST 정책 테스트로 고정 |
...
현재까지 guild channel settings helper의 `util/guild/` 패키지 이동과 Music/YouTube subscriptions/MapleStory/LoL scrim/DDAY/Channel settings/Party/Loop/Voice chat/Search/Questions/Explanation/Interpret/Translation/Summarize/Rank/Scheduler/Status API/Celebration update/Exchange rate/Foreign reserves/Clean/Custom help/Emoji enlarge/Ping Cog package 이동이 완료되었다. root `util/music*.py`, root `util/youtube*.py`, root `util/maplestory*.py`, root loop helper, root celebration helper는 더 이상 남지 않았다.
...
- `cogs/`: Discord 명령과 백그라운드 태스크 구현
  ...
  - 패키지 Cog: `cogs/celebration_update/`, `cogs/channel_settings/`, `cogs/clean/`, `cogs/custom_help/`, `cogs/dday/`, `cogs/emoji_enlarge/`, `cogs/exchange_rate/`, `cogs/explanation/`, `cogs/foreign_reserves/`, `cogs/gambling/`, `cogs/interpret/`, `cogs/lol_scrim/`, `cogs/loop/`, `cogs/maplestory/`, `cogs/music/`, `cogs/party/`, `cogs/ping/`, `cogs/questions/`, `cogs/rank/`, `cogs/scheduler/`, `cogs/search/`, `cogs/status_api/`, `cogs/summarize/`, `cogs/translation/`, `cogs/voice_chat/`, `cogs/youtube_subscriptions/`
...
- `util/guild/channel_settings.py`, `util/db.py`, `util/youtube/subscriptions.py`처럼 반복되는 저장소 접근을 공통 유틸로 모은 지점이 있다.
...
- `cogs/status_api/__init__.py`의 WebSub 검증은 `YOUTUBE_WEBSUB_VERIFY_TOKEN`이 없으면 모든 요청을 허용한다.
...
| 작업트리 최신 검증 | `python -m compileall -q bot.py api cogs common func util test scripts` 통과, `python -m unittest discover -s test` -> 554개 통과 | 구현 진행 후 회귀 확인 |
...
| music progress helper 분리 | `util/music/progress.py` 39줄, `cogs/music/__init__.py` 현재 1706줄, progress 대상 테스트 5개 통과, `test_music*.py` 214개 통과 | 시간/진행률 UI helper를 music Cog 본문에서 분리하고, root `util/music_progress.py` 제거를 테스트로 고정 |
| music search helper/action/yt-dlp executor/response/flow/play-branch helper 분리 | `util/music/search.py` 115줄, `cogs/music/__init__.py` 현재 1706줄, search 대상 테스트 13개 및 command surface 대상 테스트 50개 통과, `test_music*.py` 215개 통과 | HTTP URL 판별, 검색 결과 URL 정규화, watch entry filtering, 검색 결과 embed title/description 조립, `/재생` 검색과 즐겨찾기 검색의 빈 결과 메시지/표시 payload action, `ytsearch10:` query 조립과 yt-dlp executor scheduling, 검색 결과 없음/Embed/View/ephemeral response sequence, 검색 실행과 action mapping flow, `/재생` 키워드 검색 분기 위임 helper를 music Cog 검색 분기에서 분리하고, root `util/music_search.py` 제거를 테스트로 고정 |
| music embed helper 분리 | `util/music/embeds.py` 97줄, `cogs/music/__init__.py` 현재 1706줄, embed 대상 테스트 4개 통과, `test_music*.py` 214개 통과 | 기본 패널/재생 중 embed 생성 UI 책임을 music Cog 본문에서 분리하고, root `util/music_embeds.py` 제거를 테스트로 고정 |
| music extractor helper 분리 | `util/music/extractor.py` 47줄, `cogs/music/__init__.py` 현재 1706줄, extractor 대상 테스트 11개 통과, `test_music*.py` 216개 통과 | yt-dlp 포맷 후보 우선순위, keyword search URL 결정, entries selection 로직을 music Cog 본문에서 분리하고, root `util/music_extractor.py` 제거를 테스트로 고정 |
| music View/Modal 분리 | `util/music/views.py` 367줄, `cogs/music/__init__.py` 현재 1706줄, views 대상 테스트 4개 통과, `test_music*.py` 227개 통과 | 검색 결과/즐겨찾기/control/helper View와 search/seek modal을 music Cog 본문에서 분리하고, root `util/music_views.py` 제거를 테스트로 고정 |
| music panel store 분리 | `util/music/panel_store.py` 44줄 추출, `cogs/music/__init__.py` 현재 1706줄, panel store 대상 테스트 5개 통과, `test_music*.py` 225개 통과 | panel message id 로드/저장/삭제 DB 접근을 music Cog 본문에서 분리하고, root `util/music_panel_store.py` 제거를 테스트로 고정 |
| music favorite snapshot/payload/play/play-request/save-entry/save-side-effect/save-response/panel-refresh/cache-load/cache-hit/cache-store/cache-apply/load-failure/manager/modal/current-button/current-track/search-request/manager-open/current-player-wrapper helper 분리 | `util/music/favorites.py` 현재 517줄, `cogs/music/__init__.py` 현재 1706줄, favorite 대상 테스트 48개 및 command surface 대상 테스트 50개 통과, `test_music*.py` 226개 통과 | 현재 재생 player를 즐겨찾기 snapshot으로 바꾸고, 검색 결과 entry와 현재 재생 snapshot을 저장 payload/action으로 정규화하며, 저장 DB side effect와 완료 메시지 결과 생성을 util helper로 이동하고, 저장 결과 이후 favorites reload, 패널 refresh, 사용자 응답 정책과 패널 refresh skip/playing/helper 모드 선택, 캐시 hit/refresh bypass 판단, cache hit 반환 payload, cache 저장 payload 생성, cache 저장 side effect/반환값, 로드 실패 fallback payload 생성을 action/helper로 고정하고 현재곡 저장 wrapper를 제거했으며, 빈 슬롯/재생 URL action 판단, 즐겨찾기 재생 URL voice guard/playback state/player preparation, 즐겨찾기 재생 요청 슬롯 검증, 검색 결과 저장 action payload 생성, manager 슬롯 선택/관리자 open 상태 계산, favorite 검색 modal/submit 정규화, 현재곡 저장 버튼 disabled/slot 판정, 현재 재생곡 없음 메시지/저장 payload 생성, 검색 저장 요청 슬롯/검색어 검증을 music Cog/View 본문에서 분리하고, root `util/music_favorites.py` 제거를 테스트로 고정 |
| music state helper 분리 | `util/music/state.py` 55줄 추출, `cogs/music/__init__.py` 현재 1706줄, state 대상 테스트 6개 통과, `test_music*.py` 223개 통과 | `GuildMusicState`와 playback/idle reset/playback start/track finish 상태 전이를 music Cog 본문에서 분리하고, root `util/music_state.py` 제거를 테스트로 고정 |
| music voice helper 분리 | `util/music/voice.py` 78줄 추출, `cogs/music/__init__.py` 현재 1706줄, voice 대상 테스트 12개 통과, `test_music*.py` 224개 통과 | 음성 연결, 채널 이동, 재생/일시정지 활성 상태 판단, 같은 음성 채널 guard, 연결 transition debug를 music Cog 본문에서 분리하고, root `util/music_voice.py` 제거를 테스트로 고정 |
| music source preparation/playback payload helper 분리 | `util/music/playback.py` 113줄, `cogs/music/__init__.py` 현재 1706줄, playback 대상 테스트 9개 통과, `test_music*.py` 219개 통과 | 일반 재생, 즐겨찾기 재생, seek, loop fallback refresh, 다음 대기열 곡 준비의 `YTDLSource.from_url` 호출과 FFmpeg/스트림 준비 실패 매핑, loop/skip replay source 재사용, prepared player의 source/확인 메시지 payload와 prepared playback side effect sequence를 단일화하고, root `util/music_playback.py` 제거를 테스트로 고정 |
| music stream extraction helper 분리 | `util/music/stream.py` 51줄, `cogs/music/__init__.py` 현재 1706줄, stream 대상 테스트 5개 통과, `test_music*.py` 217개 통과 | HTML fallback의 `ytInitialPlayerResponse` 파싱, 최고 bitrate audio URL 선택, stream metadata 구성을 music Cog 본문에서 분리하고, root `util/music_stream.py` 제거를 테스트로 고정 |
...
| music source facade 분리 | `util/music/source.py` 306줄, `cogs/music/__init__.py` 현재 1706줄, source 대상 테스트 4개 통과, `test_music*.py` 218개 통과 | `YTDLSource`, yt-dlp fallback 전략, FFmpeg 옵션 조립, HTML stream fallback 호출을 music Cog 본문에서 분리하고, root `util/music_source.py` 제거를 테스트로 고정 |
...
| MapleStory Cog package 이동 | `cogs/maplestory/__init__.py` 117줄, MapleStory events 대상 테스트 17개 통과 | 메이플 명령 표면을 package cog로 이동하고, `cogs.maplestory` extension/import 호환과 root `cogs/maplestory.py` 제거를 테스트로 고정 |
...
| Loop Cog package 이동 | `cogs/loop/__init__.py` 269줄, lifecycle 대상 테스트 5개 통과 | loop 명령/백그라운드 태스크 표면을 package cog로 이동하고, `cogs.loop` extension/import 호환과 root `cogs/loop.py` 제거를 테스트로 고정 |
...
| WebSub subscription helper 분리 | `util/youtube/websub_subscription.py` 151줄, `cogs/loop/__init__.py` 현재 269줄, WebSub subscription 대상 테스트 7개 통과 | callback URL 구성, subscribe/unsubscribe 요청, live/upload 구독 대상 필터링, WebSub 상태 저장을 loop Cog 본문에서 분리하고, root `util/youtube_websub_subscription.py` 제거를 테스트로 고정 |
| WebSub notification handler 분리 | `util/youtube/websub_notification.py` 49줄, `cogs/loop/__init__.py` 현재 269줄, WebSub notification 대상 테스트 2개 통과 | Atom notification 파싱, 구독 조회, 후보 처리 결과 집계를 loop Cog 본문에서 분리하고, root `util/youtube_websub_notification.py` 제거를 테스트로 고정 |
| YouTube community polling helper 분리 | `util/youtube/community_polling.py` 52줄, `cogs/loop/__init__.py` 현재 269줄, community polling 대상 테스트 4개 통과 | 커뮤니티 post fetch, fetch 실패 warning, notification processing 위임을 loop Cog 본문에서 분리하고, root `util/youtube_community_polling.py` 제거를 테스트로 고정 |
...
| YouTube video status helper 분리 | `util/youtube/video_status.py` 28줄, `cogs/loop/__init__.py` 현재 269줄, video status 대상 테스트 3개 통과 | Google `videos.list` 요청과 응답 classification을 loop Cog 본문에서 분리하고, root `util/youtube_video_status.py` 제거를 테스트로 고정 |
...
| YouTube community notification 분리 | `util/youtube/community_notification.py` 136줄, `cogs/loop/__init__.py` 현재 269줄, community notification 대상 테스트 6개 통과 | 커뮤니티 embed/message build, 최초 post ID seed, 새 게시물 알림 전송과 상태 저장을 loop Cog 본문에서 분리하고, root `util/youtube_community_notification.py` 제거를 테스트로 고정 |
| YouTube Atom feed fallback 분리 | `util/youtube/feed_fallback.py` 160줄, `cogs/loop/__init__.py` 현재 269줄, feed fallback 대상 테스트 5개 통과 | Atom feed polling throttle, feed fetch, seen update tracking, 후보 dispatch/refetch를 loop Cog 본문에서 분리하고, root `util/youtube_feed_fallback.py` 제거를 테스트로 고정 |
| YouTube loop runner 분리 | `util/youtube/loop_runner.py` 94줄, `cogs/loop/__init__.py` 현재 269줄, runner 대상 테스트 3개 통과 | YouTube 후보 확인, pending check timestamp helper 호출, 커뮤니티 task orchestration을 loop Cog 본문에서 분리하고, root `util/youtube_loop_runner.py` 제거를 테스트로 고정 |
| YouTube video candidate runner 분리 | `util/youtube/video_candidate_runner.py` 132줄, `cogs/loop/__init__.py` 현재 269줄, video candidate 대상 테스트 4개 통과 | live/upcoming/upload/shorts candidate 상태 처리와 outcome 반환을 loop Cog 본문에서 분리하고, root `util/youtube_video_candidate_runner.py` 제거를 테스트로 고정 |
| YouTube notification sender 분리 | `util/youtube/notification_sender.py` 92줄, `cogs/loop/__init__.py` 현재 269줄, sender 대상 테스트 5개 통과 | live/upload 알림 대상 채널 resolve와 메시지 전송을 loop Cog 본문에서 분리하고, root `util/youtube_notification_sender.py` 제거를 테스트로 고정 |
| Daily refresh runner 분리 | `util/loop/daily_refresh_runner.py` 106줄, `cogs/loop/__init__.py` 현재 269줄, daily refresh 대상 테스트 3개 및 DDAY AST 계약 테스트 통과 | 기념일/DDAY/썬데이메이플/user message reset orchestration을 loop Cog 본문에서 분리하고, root `util/daily_refresh_runner.py` 제거를 테스트로 고정 |
| WebSub renewal runner 분리 | `util/youtube/websub_renewal.py` 24줄, `cogs/loop/__init__.py` 현재 269줄, WebSub renewal 대상 테스트 3개 통과 | 주기적 WebSub 갱신 호출과 성공 로그를 loop Cog 본문에서 분리하고, root `util/youtube_websub_renewal.py` 제거를 테스트로 고정 |
| Weekly 1557 report runner 분리 | `util/loop/weekly_1557_reporter.py` 66줄, `cogs/loop/__init__.py` 현재 269줄, weekly 1557 대상 테스트 4개 통과 | 주간 1557 리포트 생성, 전송, 카운트 초기화 orchestration을 loop Cog 본문에서 분리하고, root `util/weekly_1557_reporter.py` 제거를 테스트로 고정 |
| Presence status helper 분리 | `util/loop/presence_status.py` 20줄, `cogs/loop/__init__.py` 현재 269줄, presence 대상 테스트 4개 통과 | USER_MESSAGES 캐시 집계와 Discord presence 문구 생성을 loop Cog 본문에서 분리하고, root `util/presence_status.py` 제거를 테스트로 고정 |
| MapleStory notice loop runner 분리 | `util/maplestory/notice_loop_runner.py` 47줄, `cogs/loop/__init__.py` 현재 269줄, MapleStory notice loop 대상 테스트 3개 통과 | 공지 refresh 결과 집계, 실패 로그, 전송 건수 로그를 loop Cog 본문에서 분리하고 3분 polling 위치는 유지하며, root `util/maplestory_notice_loop_runner.py` 제거를 테스트로 고정 |
...
| DDAY Cog package 이동 | `cogs/dday/__init__.py` 236줄, DDAY 대상 테스트 13개 통과 | DDAY 명령 표면을 package cog로 이동하고, `cogs.dday` extension/import 호환과 root `cogs/dday.py` 제거를 테스트로 고정 |
| Guild channel settings helper 패키지 이동 | `util/guild/channel_settings.py` 44줄, guild channel settings 대상 테스트 3개 통과 | guild별 기능 채널 설정 read/write helper를 guild 카테고리 패키지로 이동하고, root `util/channel_settings.py` 제거를 테스트로 고정 |
| Channel settings Cog package 이동 | `cogs/channel_settings/__init__.py` 142줄, music command surface 대상 테스트 51개 통과 | guild/channel 설정 명령 표면을 package cog로 이동하고, `cogs.channel_settings` extension/import 호환과 root `cogs/channel_settings.py` 제거를 테스트로 고정 |
| Party Cog package 이동 | `cogs/party/__init__.py` 365줄, party layout 대상 테스트 2개 통과 | 파티 명령 표면을 package cog로 이동하고, `cogs.party` extension/import 호환과 root `cogs/party.py` 제거를 테스트로 고정 |
...
| Interpret Cog package 이동 | `cogs/interpret/__init__.py` 272줄, interpret layout 대상 테스트 2개 통과 | 해석 명령/메시지 해석 컨텍스트 메뉴 표면을 package cog로 이동하고, `cogs.interpret` extension/import 호환과 root `cogs/interpret.py` 제거를 테스트로 고정 |
| Translation Cog package 이동 | `cogs/translation/__init__.py` 202줄, translation layout 대상 테스트 2개 통과 | 번역 명령/메시지 번역 컨텍스트 메뉴 표면을 package cog로 이동하고, `cogs.translation` extension/import 호환과 root `cogs/translation.py` 제거를 테스트로 고정 |
| Summarize Cog package 이동 | `cogs/summarize/__init__.py` 230줄, summarize layout 대상 테스트 2개 통과 | 대화요약/유튜브 요약 명령과 유튜브 요약 컨텍스트 메뉴 표면을 package cog로 이동하고, `cogs.summarize` extension/import 호환과 root `cogs/summarize.py` 제거를 테스트로 고정 |
| Rank Cog package 이동 | `cogs/rank/__init__.py` 233줄, rank layout 대상 테스트 2개 통과 | Riot 랭크 명령과 일일 랭크 루프 명령 표면을 package cog로 이동하고, `cogs.rank` extension/import 호환과 root `cogs/rank.py` 제거를 테스트로 고정 |
| Scheduler Cog package 이동 | `cogs/scheduler/__init__.py` 227줄, scheduler layout 대상 테스트 2개 통과 | 예약 명령 그룹과 일반/반복/리스트 하위 명령 표면을 package cog로 이동하고, `cogs.scheduler` extension/import 호환과 root `cogs/scheduler.py` 제거를 테스트로 고정 |
| Status API Cog package 이동 | `cogs/status_api/__init__.py` 214줄, status API layout 대상 테스트 2개 통과 | health/celebration/WebSub HTTP route 표면을 package cog로 이동하고, `cogs.status_api` extension/import 호환과 root `cogs/status_api.py` 제거를 테스트로 고정 |
| Celebration update Cog package 이동 | `cogs/celebration_update/__init__.py` 56줄, celebration update layout 대상 테스트 2개 통과 | 기념일 수동 갱신 명령 표면을 package cog로 이동하고, `cogs.celebration_update` extension/import 호환과 root `cogs/celebration_update.py` 제거를 테스트로 고정 |
| Exchange rate Cog package 이동 | `cogs/exchange_rate/__init__.py` 279줄, exchange rate layout 대상 테스트 2개 통과 | 환율 명령 표면을 package cog로 이동하고, `cogs.exchange_rate` extension/import 호환과 root `cogs/exchange_rate.py` 제거를 테스트로 고정 |
| Foreign reserves Cog package 이동 | `cogs/foreign_reserves/__init__.py` 195줄, foreign reserves layout 대상 테스트 2개 통과 | 외환보유액 명령 표면을 package cog로 이동하고, `cogs.foreign_reserves` extension/import 호환과 root `cogs/foreign_reserves.py` 제거를 테스트로 고정 |
| Clean Cog package 이동 | `cogs/clean/__init__.py` 99줄, clean layout 대상 테스트 2개 통과 | 관리자 메시지 정리 명령 표면을 package cog로 이동하고, `cogs.clean` extension/import 호환과 root `cogs/clean.py` 제거를 테스트로 고정 |
| Custom help Cog package 이동 | `cogs/custom_help/__init__.py` 213줄, custom help layout 대상 테스트 2개 통과 | 도움말/기가채드 명령 표면을 package cog로 이동하고, `cogs.custom_help` extension/import 호환과 root `cogs/custom_help.py` 제거를 테스트로 고정 |
| Emoji enlarge Cog package 이동 | `cogs/emoji_enlarge/__init__.py` 41줄, emoji enlarge layout 대상 테스트 2개 통과 | 단일 커스텀 이모지 확대 listener 표면을 package cog로 이동하고, `cogs.emoji_enlarge` extension/import 호환과 root `cogs/emoji_enlarge.py` 제거를 테스트로 고정 |
| Ping Cog package 이동 | `cogs/ping/__init__.py` 17줄, ping layout 대상 테스트 2개 통과 | 핑 명령 표면을 package cog로 이동하고, `cogs.ping` extension/import 호환과 root `cogs/ping.py` 제거를 테스트로 고정 |
