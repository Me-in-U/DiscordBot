...
9. 현재 최신 검증은 `compileall` 통과, unittest 410개 통과다.
10. 이번 패치로 music favorite search request action 분리가 구현되었고, 다음 후보는 music favorite manager open action 분리다.
...
최신 검증 결과: `python -m compileall -q bot.py api cogs common func util test scripts` 통과, `python -m unittest discover -s test` 410개 통과.
...
이번 패치로 music favorite search request action 분리가 완료되었다. 다음 작은 후보는 music favorite manager open action 분리다.
...
| 작업트리 최신 검증 | `python -m compileall -q bot.py api cogs common func util test scripts` 통과, `python -m unittest discover -s test` -> 410개 통과 | 구현 진행 후 회귀 확인 |
...
| music favorite snapshot/payload/play/manager/modal/current-button/current-track/search-request helper 분리 | `util/music_favorites.py` 현재 431줄, `cogs/music.py` 현재 1681줄, favorite 대상 테스트 31개 통과 | 현재 재생 player를 즐겨찾기 snapshot으로 바꾸고, 검색 결과 entry와 현재 재생 snapshot을 저장 payload/action으로 정규화하며, 빈 슬롯/재생 URL action 판단, manager 슬롯 선택 상태 계산, favorite 검색 modal/submit 정규화, 현재곡 저장 버튼 disabled/slot 판정, 현재 재생곡 없음 메시지/저장 payload 생성, 검색 저장 요청 슬롯/검색어 검증을 music Cog/View 본문에서 분리 |
