...
9. 현재 최신 검증은 `compileall` 통과, unittest 455개 통과다.
10. 이번 패치로 music search helper가 `util/music/` 카테고리 패키지로 이동되었고, 다음 후보는 music extractor helper의 `util/music/` 패키지 이동이다.
...
최신 검증 결과: `python -m compileall -q bot.py api cogs common func util test scripts` 통과, `python -m unittest discover -s test` 455개 통과.
...
이번 패치로 music search helper의 `util/music/` 패키지 이동이 완료되었다. 다음 작은 후보는 music extractor helper의 `util/music/` 패키지 이동이다.
...
| 작업트리 최신 검증 | `python -m compileall -q bot.py api cogs common func util test scripts` 통과, `python -m unittest discover -s test` -> 455개 통과 | 구현 진행 후 회귀 확인 |
...
| music playback/skip/seek/URL action helper 분리 | `util/music_playback_actions.py` 156줄, `cogs/music.py` 현재 1706줄, playback action 대상 테스트 17개 통과 | 일시정지/다시재생/정지/반복/스킵/구간 이동에 더해 URL 재생의 active voice 대기열 추가/즉시 준비 판단, play_url_now replacement 상태 전이, queue track 반환, 사용자 응답 문구를 command handler에서 분리 |
...
| music extractor helper 분리 | `util/music_extractor.py` 61줄 추출, `cogs/music.py` 현재 1706줄, extractor 대상 테스트 10개 통과 | yt-dlp 포맷 후보 우선순위, keyword search URL 결정, entries selection 로직을 music Cog 본문에서 분리 |
...
| music panel store 분리 | `util/music_panel_store.py` 58줄 추출, `cogs/music.py` 현재 1706줄, panel store 대상 테스트 4개 통과 | panel message id 로드/저장/삭제 DB 접근을 music Cog 본문에서 분리 |
...
| music stream extraction helper 분리 | `util/music_stream.py` 61줄 추출, `cogs/music.py` 현재 1706줄, stream 대상 테스트 4개 추가 | HTML fallback의 `ytInitialPlayerResponse` 파싱, 최고 bitrate audio URL 선택, stream metadata 구성을 music Cog 본문에서 분리 |
...
| music source facade 분리 | `util/music_source.py` 354줄 추출, `cogs/music.py` 현재 1706줄, source 대상 테스트 3개 추가, `test_music*.py` 207개 통과 | `YTDLSource`, yt-dlp fallback 전략, FFmpeg 옵션 조립, HTML stream fallback 호출을 music Cog 본문에서 분리 |
