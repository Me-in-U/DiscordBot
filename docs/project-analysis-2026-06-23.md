...
9. 현재 최신 검증은 `compileall` 통과, unittest 502개 통과다.
10. 이번 패치로 guild channel settings helper가 `util/guild/` 카테고리 패키지로 이동되었고, root util은 공통 infra 중심으로 축소되었다.
...
최신 검증 결과: `python -m compileall -q bot.py api cogs common func util test scripts` 통과, `python -m unittest discover -s test` 502개 통과.
...
| 작업트리 최신 검증 | `python -m compileall -q bot.py api cogs common func util test scripts` 통과, `python -m unittest discover -s test` -> 502개 통과 | 구현 진행 후 회귀 확인 |
